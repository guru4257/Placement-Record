...
describe('TodoService', () => {
  ...
  it('should return expected todos and HttpClient called once', (done: DoneFn) => {
    const httpGetAllTodosResult: any[] =
      [{ id: 1, label: 'A', description: 'D', category: TodoCategory.Software, done: false },
      { id: 2, label: 'A2', description: 'D2', category: TodoCategory.Hardware, done: '22-10-2019' }];

    const expectedTodos: TodoItem[] = [
      new TodoItem(1, "A", "D", TodoCategory.Software, false, null),
      new TodoItem(2, "A2", "D2", TodoCategory.Hardware, true, new Date(2019, 9, 22))
    ]
  ...
  it('should add todo', (done: DoneFn) => {
    const httpAddTodoItem: any =
      { id: 1, label: 'A', description: 'D', category: TodoCategory.Software, done: false };
    const expectedTodo: TodoItem = new TodoItem(1, "A", "D", TodoCategory.Software, false, null);
